...
_.defaults exports,
  generate_seeds_from_rule: (survival = [2,3], birth = [3]) ->
    ...
    # The sixteen canonical 2x2 squares. 2x2 squares do not compute results
    all_2x2_squares = cartesian_product([Cell.Dead, Cell.Alive]).map (quadrants) ->
      Square.cache.add new Square(quadrants)

    # The 65,536 canonical 4x4 squares. 4x4 squares know how to generate
    # their 2x2 results, and as we saw above, they memoize those results
    # so that they are only computed once.
    #
    # (A variation of memoizing the result computation is to compute it when
    # generating the 4x4 square, thus "compiling" the supplied rules into a table of
    # 65,536 rules taht is looked up at runtime.)
    #
